...
const loadPortfolioData = async () => {
  try {
    ...
    if (stackGrid) {
      const stackRes = await fetch('data/skills.json');
      if (stackRes.ok) {
        const skills = await stackRes.json();
        skills.forEach((skill, index) => {
          const items = skill.items.map(item => "<span style=\"white-space: nowrap;\">" + esc(item) + "</span>").join('<span class="stack-divider">/</span> ');
